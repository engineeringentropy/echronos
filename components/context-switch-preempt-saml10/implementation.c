 ...
 */
#define SAML10_CONTEXT_FIRST_RUN_BIT 0x1
#define SAML10_CONTEXT_FIRST_RUN_MASK (~(0x1))

/*| types |*/
typedef uint32_t saml10_run_flags_t;
typedef struct saml10_task_context_layout_t* context_t;
             ...
             uint32_t *const stack_base, const size_t stack_size)
{
     ...
     */
    uint32_t* stack_top = stack_base + stack_size - saml10_CONTEXT_SIZE;
    *ctx = (context_t)stack_top;

    struct saml10_task_context_layout_t* context = *ctx;
    context->low.PC = (uint32_t)fn;
    context->low.xPSR = (uint32_t)1 << 24;
    context->high.runFlags = SAML10_CONTEXT_FIRST_RUN_BIT;
}
