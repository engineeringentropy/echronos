/* Note: The ARMv8 architecture is a bit different.
 * This is cribbed from the saml10 implementation.
 */
...
/* 
struct saml10_exception_additional_t {
    uint32_t R8;
    uint32_t R9;
    uint32_t R10;
    uint32_t R11;
    saml10_run_flags_t runFlags;
    uint32_t R4;
    uint32_t R5;
    uint32_t R6;
    uint32_t R7;
...
struct saml10_exception_context_stack_t {
    uint32_t R0;
    uint32_t R1;
    uint32_t R2;
    uint32_t R3;
...
struct saml10_task_context_layout_t {
    struct saml10_exception_additional_t high;
    struct saml10_exception_context_stack_t low;
};
 */
...
rtos_internal_context_switch_first:
        ...
        ldr r1, [r0]
        /* PC is #60 and comes from saml10_exception_context_stack_t */
        ldr r3, [r1,#60]
        add r1, #68
         ...
         * This must be the same as MSPStack in saml10-exceptions.c
...
/* Implements the functionality of rtos_internal_yield. */
         ...
         *  - dereference pointer to retrieve a pointer to saml10_task_context_layout_t for the old task
         *  - dereference pointer to retrieve a pointer to saml10_task_context_layout_t for the new task
        ...
        mrs r3, psp
        /* r3 = pointer to saml10_task_context_layout_t for the old task */        
